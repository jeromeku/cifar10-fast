...
def _maybe_make_path(fp):
    if not os.path.exists(fp):
        os.makedirs(fp)
        print(f"Creating path {fp}")


class TableLoggerV2():
    def __init__(self, table=None, save_path='/tmp/logs/'):
        self.table = table or []
    ...
    def append(self, output):
        if not hasattr(self, 'keys'):
            self.keys = output.keys()
            self.add_row(*(f'{k:>12s}' for k in self.keys))
        filtered = [output[k] for k in self.keys]
        self.summaries.append(output)
        self.add_row(*(f'{v:12.4f}' if isinstance(v, np.float)
                       else f'{v:12}' for v in filtered))

    def add_row(self, *cols, sep='', show=True):
        row = f"{sep}".join(cols)
    ...
    def save(self, fn=None):
        fn = fn or 'table.log'
        _maybe_make_path(self.save_path)
        full_path = os.path.join(self.save_path, fn)
...
def build_graph(net):
    ...
    def with_default_inputs(vals): return (val if isinstance(val, tuple) else (
...
def train(model, optimizer, train_batches, test_batches, epochs,
    ...
    for epoch in range(epochs):
        ...
        summary = union({'epoch': epoch+1, 'lr': optimizer.param_values()
                         ['lr']*train_batches.batch_size}, epoch_stats)
...
def make_pydot(nodes, edges, direction='LR', sep=sep, **kwargs):
    ...
    def parent(path): return path[:-1]

    def stub(path): return path[-1]

    class Subgraphs(dict):
        def __missing__(self, path):
            subgraph = pydot.Cluster(sep.join(path), label=stub(
...
def get_params(mod): return {p.name: getattr(mod, p.name, '?')
...
def walk(dict_, key): return walk(dict_, dict_[key]) if key in dict_ else key
